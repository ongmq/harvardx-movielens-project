...
```{r downloading packages, message=FALSE}
...
```{r creating validation set, warning=FALSE}
...
```{r cleaning up validation set, message=FALSE}
...
```{r removing datasets-1, include=FALSE}
...
```{r simple summary statistics, echo=FALSE}
...
```{r avg rating of users, echo=FALSE}
edx %>%
  group_by(userId) %>%
...
```{r no. of movies and avg rating for each genre}
...
```{r visualising genre effect, include=FALSE}
...
```{r changing date format, message=FALSE}
...
```{r visualising time effect, echo=FALSE}
...
```{r creating test set, warning=FALSE}
set.seed(1, sample.kind = "Rounding")
...
```{r cleaning up test set, message=FALSE}
test_set <- temp %>% 
...
```{r removing datasets-2, include=FALSE}
...
```{r optimal lambda-2, echo=FALSE}
lambda <- lambdas[which.min(rmses)]
lambda
```

```{r calculating final rmse}
